...
public class NaSoEBeListAdapter extends BaseAdapter {
	...
	public NaSoEBeListAdapter(Context ctx,String Language){
		mctx = ctx;
		language = Language;
	...
	protected void  pullData() {
		...
		mdbHelper.open();
		Cursor c = mdbHelper.fetchAllNaSoBySeasonAndLanguage("1", language);
		c.moveToFirst();
		mcount = c.getCount();
		...
		for (int i = 0; i < mcount; i++) {
			...
			imgs[i] = fl.getImage("Naso"+vidids[i].trim());
		}
	}
