...
public class ShortFilmListAdapter extends BaseAdapter {
	...
	public ShortFilmListAdapter(Context ctx,String Language){
		mctx = ctx;
		language = Language;
	...
	protected void  pullData() {
		...
		mdbHelper.open();
		Cursor c = mdbHelper.fetchAllShortFilmBySeasonAndLanguage("1", language);
		c.moveToFirst();
		mcount = c.getCount();
		...
		for (int i = 0; i < mcount; i++) {
			...
			imgs[i] = fl.getImage("SF"+vidids[i].trim());
		}
	}
